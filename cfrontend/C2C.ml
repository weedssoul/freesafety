...
let typeStringLiteral s =
  Tarray(Tint(I8, Unsigned, noattr), Z.of_uint (String.length s + 1), Ownership.empty(),  noattr)
...
let va_list_ptr e =
  if not CBuiltins.va_list_scalar then e else
    match e with
    | Evalof(e', _) -> Eaddrof(e', Tpointer(typeof e, Ownership.empty (), noattr))
...
let make_builtin_va_arg env ty e =
  ...
  let ty_fun =
    (* TODO: ok? *)
    Tfunction(Tcons(Tpointer(Tvoid, Ownership.empty(), noattr), Tnil),
              Tcons(Tpointer(Tvoid, Ownership.empty(), noattr), Tnil),
              ty_ret, cc_default) in
  Ecast
...
let mergeTypAttr ty a2 =
  ...
  | Tpointer(ty', o, a1) -> Tpointer(ty', o, mergeAttr a1 a2)
  | Tarray(ty', sz, o, a1) -> Tarray(ty', sz, o, mergeAttr a1 a2)
  | Tfunction(targs, _, tres, cc) -> ty
  | Tstruct(id, fld, a1) -> Tstruct(id, fld, mergeAttr a1 a2)
  | Tunion(id, fld, a1) -> Tunion(id, fld, mergeAttr a1 a2)
  | Tcomp_ptr(id, a1, o, i) -> Tcomp_ptr(id, mergeAttr a1 a2, o, i)
  | Tplus _ -> ty
...
let convertTyp env t =
  ...
  let rec convertTyp seen t =
    ...
    | C.TArray(ty, None, a) ->
        ...
        Tarray(convertTyp seen ty, coqint_of_camlint 0l, Ownership.empty(), convertAttr a)
    | C.TArray(ty, Some sz, a) ->
        Tarray(convertTyp seen ty, convertInt sz, Ownership.empty(), convertAttr a)
 ...
 | C.ECall({edesc = C.EVar {name = "__builtin_memcpy_aligned"}}, args) ->
  ...
  | C.ECall({edesc = C.EVar {name = "__builtin_va_copy"}}, [arg1; arg2]) ->
      ...
      Ebuiltin(EF_memcpy(Z.of_uint CBuiltins.size_va_list, Z.of_uint 4),
               Tcons(Tpointer(Tvoid, Ownership.empty(), noattr),
                 Tcons(Tpointer(Tvoid, Ownership.empty(), noattr), Tnil)),
...
and convertLvalue env e =
  ...
  | C.EUnop(C.Oarrow id, e1) ->
      ...
      let ty1 =
        ...
        | Tpointer(t, _, _) | Tarray(t, _, _, _) -> t
...
let convertFundecl env (sto, id, ty, optinit) =
  let (args, res, cconv) =
    ...
    | Tfunction(args, _, res, cconv) -> (args, res, cconv)
    | _ -> assert false in
