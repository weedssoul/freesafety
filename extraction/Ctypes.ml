...
| Tpointer of coq_type * O.ownership * attr
| Tarray of coq_type * coq_Z * O.ownership * attr
| Tfunction of typelist * typelist * coq_type * calling_convention
| Tstruct of ident * fieldlist * attr
| Tunion of ident * fieldlist * attr
| Tcomp_ptr of ident * attr * O.ownership * int
| Tplus of (ident * int) list * O.ownership
and typelist =
| Tnil
...
let rec type_eq =
  ...
  | Tpointer (t, _, a) ->
    (match ty2 with
     | Tpointer (t0, _, a0) -> if type_eq t t0 then h2 a a0 else false
     | _ -> false)
  | Tarray (t, z, _, a) ->
    (match ty2 with
     | Tarray (t0, z0, _, a0) ->
  ...
  | Tfunction (t, t', t0, c) ->
    (match ty2 with
     | Tfunction (t1, t1', t2, c0) ->
       if typelist_eq t t1 && typelist_eq t' t1'
  ...
  | Tcomp_ptr (i, a, _, _) ->
    (match ty2 with
     | Tcomp_ptr (i0, a0, _, _) -> if ident_eq i i0 then h2 a a0 else false
     | _ -> false)
  | Tplus _ -> false
)
...
| Tpointer (elt, o, a) -> Tpointer (elt, o, (f a))
| Tarray (elt, sz, o, a) -> Tarray (elt, sz, o, (f a))
| Tstruct (id, fld, a) -> Tstruct (id, fld, (f a))
| Tunion (id, fld, a) -> Tunion (id, fld, (f a))
| Tcomp_ptr (id, a, o, i) -> Tcomp_ptr (id, (f a), o, i)
| _ -> ty
...
(****************************************************************
In Tpointer(ty, o, attr), o should be ignored if ty is a function type
since a function pointer cannot be passed to free nor should not be
seen as leaky.
 ****************************************************************)
...
| Tarray (t, sz, o, a) -> Tpointer (t, o, noattr)
| Tfunction (t, t', t0, c) -> Tpointer (ty, O.empty (), noattr)
    ...
    (match t with
     ...
     | Tint (i, s, a) ->
       (match i with
        ...
        | _ -> Zpos Coq_xH)
     | Tarray (t', z, _, a) -> alignof t'
     | Tfunction (t0, _, t1, c) -> Zpos Coq_xH
...
| Tpointer (t0, _, a) -> Zpos (Coq_xO (Coq_xO Coq_xH))
| Tarray (t', n, _, a) -> Z.mul (sizeof t') (Z.max Z0 n)
| Tstruct (i, fld, a) -> align (sizeof_struct fld Z0) (alignof t)
| Tunion (i, fld, a) -> align (sizeof_union fld) (alignof t)
| Tcomp_ptr (i, a, _, _) -> Zpos (Coq_xO (Coq_xO Coq_xH))
| _ -> Zpos Coq_xH
...
| Tpointer (t, _, a) -> By_value Mint32
| Tarray (t, z, _, a) -> By_reference
| Tfunction (t, _, t0, c) -> By_reference
...
| Tfloat (f, a) ->
  (match f with
   ...
   | F64 -> Zpos (Coq_xO (Coq_xO (Coq_xO Coq_xH))))
| Tpointer (t0, _, a) -> Zpos (Coq_xO (Coq_xO Coq_xH))
| Tarray (t', z, _, a) -> alignof_blockcopy t'
| Tstruct (i, fld, a) ->
