...
let make_set id l =
  ...
  | Some chunk ->
    let typtr = Tpointer ((Clight.typeof l), empty (), noattr) in
...
let make_assign l r =
  ...
  | Some chunk ->
    ...
    let typtr = Tpointer (ty, empty (), noattr) in
...
(** assign_fresh_ovar : (ident * coq_type) list -> (ident * coq_type) list .
    assign fresh ownership variable to Tpointer, Tarray, Tcomp_attr **)
...
let rec assign_fresh_ovar (l : (ident * coq_type) list) (r : (ident * coq_type) list) =
  match l with
  | [] -> r
  | (id , ty) :: tail ->
    let t = fresh_ovar_type ty in
    assign_fresh_ovar tail ((id, t) :: r)

(** val transl_function : coq_function -> Clight.coq_function mon **)

let transl_function f g =
  (* KS: Assign fresh ownership variables to each pointer type in each
  function. *)
    ...
    (match transl_stmt f.fn_body g' with
     ...
     | Res (a0, g'0) ->
       (match get_trail g'0 with
        | Err msg0 -> Err msg0
        | Res (a1, g'1) ->
          let new_params = assign_fresh_ovar f.fn_params [] in
          let new_vars = assign_fresh_ovar f.fn_vars [] in
          let new_temps = assign_fresh_ovar a1 [] in
          let new_return = fresh_ovar_type f.fn_return in
          Res ({ Clight.fn_return = new_return;
                 Clight.fn_callconv = f.fn_callconv;
                 Clight.fn_params = new_params;
                 Clight.fn_vars = new_vars;
                 Clight.fn_temps = new_temps;
                 Clight.fn_body = a0 }, g'1)))
  ...
  | p :: l' ->
    ...
    (match g0 with
     ...
     | Gvar v ->
       (match transl_globdefs l' g with
        | OK x ->
          let nt = match v.gvar_info with
            | Tpointer (ty, o, a) ->
              let fo = fresh_ovar () in
              Tpointer (ty, OVar fo, a)
            | Tarray (ty, z, o, a) ->
              let fo = fresh_ovar () in
              Tarray (ty, z, OVar fo, a)
            | _ -> v.gvar_info
          in
          OK ((id, (Gvar {gvar_info = nt;
                          gvar_init = gvar_init v;
                          gvar_readonly = gvar_readonly v;
                          gvar_volatile = gvar_volatile v})) :: x)
        | Error msg0 -> Error msg0))

(** val transl_program : program -> Clight.program res **)

(* KS: Insert typechecking here! *)
