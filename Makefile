...
CAMLINCLUDES=$(patsubst %,-I %, $(DIRS)) -I extraction -package ounit -lflags str.cmxa -cflags -annot
...
test: driver/Configuration.ml
	$(OCAMLBUILD) -no-hygiene -Is lib,extraction -cflag -lflag -g -ocamlc 'ocamlfind c -package oUnit -linkpkg' \
		extraction/typing_test.byte && ./typing_test.byte

ccomp: driver/Configuration.ml
	$(OCAMLBUILD) $(OCB_OPTIONS) driver/Driver.native \
        && rm -f ccomp && $(SLN) _build/driver/Driver.native ccomp

ccomp.prof: driver/Configuration.ml
	$(OCAMLBUILD) $(OCB_OPTIONS) driver/Driver.p.native \
...
clean:
	...
	rm -f ccomp ccomp.byte cchecklink cchecklink.byte clightgen clightgen.byte typing_test.byte
	rm -rf _build
